...
// Added activation function to avoid divergence at small scale factors
double activation(double a, double a_start, double sharpness) {
    if (a < 1e-3) return 0; // Explicitly avoid divergence at small scale factors
    return 1.0 / (1.0 + exp(-sharpness*(a - a_start)));
}
...
int background_functions(
  ...
  /* Apply dark energy - dark matter coupling if enabled */
  double coupling_factor = 0.0;
  double mg_factor = 0.0;
  
  if (pba->has_coupling == _TRUE_) {
    double z = 1.0/a - 1.0;
    double coupling_strength = pba->coupling_strength;
    
    /* Variable coupling implementation */
    if (pba->coupling_type == VARIABLE_COUPLING) {
      /* Calculate coupling strength based on redshift transition */
      double z_transition = pba->coupling_z_transition;
      double exponent = pba->coupling_exponent;
      
      /* Smooth transition using activation function */
      double transition_factor = 1.0 / (1.0 + exp(-(z - z_transition) * 3.0));
      
      /* Apply power-law scaling for coupling strength */
      if (z > z_transition) {
        coupling_strength *= pow((1.0 + z)/(1.0 + z_transition), exponent) * transition_factor;
      } else {
        coupling_strength *= (1.0 - transition_factor);
      }
    }
    
    /* Calculate coupling contribution to H_prime */
    if (pba->has_cdm == _TRUE_) {
      coupling_factor = coupling_strength * pvecback[pba->index_bg_rho_cdm] * 
                        (pvecback[pba->index_bg_w_fld] - 1.0) * a;
    }
  }
  
  /* Apply modified gravity effects if enabled */
  if (pba->use_modified_gravity == _TRUE_) {
    double z = 1.0/a - 1.0;
    double mg_Y = 1.0; // Default value (GR)
    
    /* Calculate modified gravity parameters based on parametrization */
    if (pba->mg_parametrization == OMEGA_DE_DEPENDENT) {
      /* Modification proportional to dark energy density fraction */
      double Omega_de = pvecback[pba->index_bg_rho_fld]/rho_tot;
      mg_Y = 1.0 + (pba->mg_Y_today - 1.0) * Omega_de/pba->Omega0_de;
    }
    else if (pba->mg_parametrization == REDSHIFT_DEPENDENT) {
      /* Redshift-dependent parametrization with transition */
      double z_transition = pba->mg_z_transition;
      
      /* Smooth transition using activation function */
      double transition_factor = 1.0 / (1.0 + exp(-(z - z_transition) * 3.0));
      
      /* Interpolate between GR (mg_Y=1) at high z and modified value today */
      mg_Y = 1.0 + (pba->mg_Y_today - 1.0) * (1.0 - transition_factor);
    }
    
    /* Apply modified gravity correction to H_prime */
    mg_factor = (mg_Y - 1.0) * (- (3./2.) * (rho_tot + p_tot) * a);
  }
  
  pvecback[pba->index_bg_H_prime] = - (3./2.) * (rho_tot + p_tot) * a + pba->K/a + coupling_factor + mg_factor;
...
int background_w_fld(
  ...
  case CLP:
    *w_fld = pba->w0_fld + pba->wa_fld * (1. - a);
    break;
  case CPL_PLUS:
    /* CPL+ parameterization extends the standard CPL with a transition feature */
    /* Calculate redshift from scale factor */
    z = 1.0/a - 1.0;
    /* Get transition redshift from coupling parameters */
    z_transition = pba->coupling_z_transition;
    
    /* Standard CPL at low redshift, modified at high redshift with transition */
    if (z < z_transition) {
      /* Standard CPL below transition redshift */
      *w_fld = pba->w0_fld + pba->wa_fld * (1. - a);
    } else {
      /* Modified behavior above transition redshift */
      /* Use activation function to create smooth transition */
      double transition_factor = 1.0 / (1.0 + exp(-(z - z_transition) * 3.0));
      *w_fld = pba->w0_fld + pba->wa_fld * (1. - a) * (1.0 - transition_factor * 0.5);
    }
    break;
  case EDE:
