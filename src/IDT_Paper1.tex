...
$\sln^{\rm early} = 0.15$; $\zc^{\rm late} = 1.0$, $\sln^{\rm late} 
...
effect. Its sign has a physical interpretation: $\eta_0 < 0$ 
corresponds to effective screening, in which the external domain's 
...
$\eta_0$: $\dAIC < -6$ for any $\eta_0 \in [-0.10, -0.01]$. This
is a consequence of the fact that the dominant contribution to $\dchi$
comes from the acoustic scale improvement (early domain), which is
independent of $\eta_0$.
...
clear optimum at $\fdom^{\rm early} \approx 0.03$--$0.04$. The
amplitude is constrained by the acoustic scale: too small an
amplitude fails to correct $l_a$, while too large an amplitude
overshoots and worsens other constraints.
...
epoch-separation: the early domain ($z \sim 2000$) has decayed
to negligible amplitude before the late domain ($z \sim 1$) becomes
active. It is not a result of parameter tuning.
...
% =====================================================================
% REFERENCES (placeholder — replace with .bib file)
% =====================================================================
...
\emph{Mon.\ Not.\ Roy.\ Astron.\ Soc.}\ \textbf{496}, L91 (2020).
