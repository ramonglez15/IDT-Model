...
contribution behaves as a smooth external component with negligible
perturbations in our frame. We treat domain overlap as an effective contribution to the
coarse-grained energy density within our observable patch, without
specifying a global embedding metric. This is analogous to the
backreaction approach~\cite{Buchert2000}, in which sub-horizon
inhomogeneities modify the effective Friedmann dynamics without
requiring a single globally defined background. The assumption of negligible domain
perturbations corresponds to the limit of a smooth external component
with effective sound speed $c_s^2 \approx 1$, preventing clustering
on sub-horizon scales---a standard and conservative choice for
non-clustering dark sector components. In this case, the domain modifies the 
...
Figure~\ref{fig:Hz} illustrates the fractional deviation of $H(z)$ from $\Lambda$CDM for each domain configuration.
...
$z \to 0$ and $z \to \infty$ where the domain is absent. This
represents the lowest-order local expansion in domain presence;
higher-order or derivative-dependent terms (e.g., proportional to
$\partial\rho_{\rm dom}/\partial t$) are possible but not required
by current data. For the 
...
observables. It represents a novel growth modification pathway,
motivated by IDT domain physics, that BAO and supernovae do not
constrain.
...
\subsection{Effective field interpretation of domain overlap}

The dephasing coupling $\eta_0$ is introduced as a phenomenological
parameter, but its structure is motivated by a specific physical
picture rather than arbitrary parameterization.

Consider two spacetime regions expanding at different rates that
overlap gravitationally. The total gravitational potential at any
point in our universe receives contributions from both our matter
and the external domain. As long as the overlap is static, the
external contribution is indistinguishable from additional smooth
energy density---this is the Candidate~1 (background-only) regime,
already captured by the Friedmann equation.

When the overlap is \emph{time-dependent}---as it must be if the
external domain expands at a different rate---the external
gravitational potential $\Phi_{\rm ext}$ changes on cosmological
timescales. A weakening $\Phi_{\rm ext}$ (from a domain that is
fading or separating) reduces the effective depth of the potential
wells in which our matter collapses. This is not equivalent to
simply reducing the matter density; it modifies the relationship
between the expansion rate (set by total energy) and the
clustering rate (set by the effective gravitational source
strength).

The coupling function $\eta(z) = \eta_0\,\rho_{\rm dom}/
\rho_{\rm dom,peak}$ encodes the simplest realization of this
picture: the modification is proportional to the domain's
presence (zero when the domain is absent) and parameterized by a
single coupling strength $\eta_0$. The sign $\eta_0 < 0$
corresponds to the physically expected regime in which a fading
external overlap weakens effective self-gravity.

We emphasize that this interpretation motivates the functional
form of $\eta(z)$ but does not constitute a first-principles
derivation. A complete derivation would require specifying the
overlap geometry and solving for the effective metric in the
overlap region---a task deferred to future work. The present
analysis demonstrates that the \emph{observable consequences} of
this picture are consistent with current data and preferred over
$\Lambda$CDM, establishing the empirical viability of the
mechanism.
...
physical premise of external domain interactions. To our knowledge,
this combination of epoch-localized background contributions and
independent growth modification within a minimal parameterization
has not been extensively explored in the cosmological literature.
...
The model preference over $\Lambda$CDM is robust across three
independent axes of variation:

\begin{enumerate}
\item \textbf{Dephasing coupling}: $\dAIC < -6$ for all
$\eta_0 \in [-0.10, -0.01]$ (Table~\ref{tab:eta_robust}). The
result does not require precise knowledge of the coupling strength.

\item \textbf{Early domain amplitude}: $\dAIC < -8$ for
$\fdom^{\rm early} \in [0.02, 0.06]$ (Table~\ref{tab:fearly_robust}).
The data select a preferred amplitude near $\fdom \approx
0.03$--$0.04$, constrained by the acoustic scale.

\item \textbf{MCMC initialization}: Four independent chains with
different random seeds converge to $\dAIC \in [-12.5, -10.5]$
(Table~\ref{tab:seeds}), confirming that the result is not an
artifact of chain initialization or stochastic fluctuation.
\end{enumerate}

This three-axis stability distinguishes the IDT result from models
whose statistical preference depends sensitively on specific
parameter choices or sampler configurations.
...
relative suppression of structure growth compared to the
background-only case, via the dephasing mechanism.
